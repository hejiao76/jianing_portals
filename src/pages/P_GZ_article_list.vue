<template>
  <div class="global_box">
    <div class="global_box_top">
      <v-header></v-header>
      <v-header-nav v-bind:menuTag="2"></v-header-nav>
    </div>
    <!--公用顶部end-->
    <div class="container call-foreman">
      <div class="tNavWrop">
        <div>所在城区:</div>
      ...
      <div class="tNavWrop">
        ...
        <span class="tNav" v-bind:class="{'active' : typeCode ===1 }" v-on:click="typeClick(1)">家装</span>
        <span class="tNav" v-bind:class="{'active' : typeCode ===2 }" v-on:click="typeClick(2)">工装</span>
      </div>
      <div class="tNavWrop">
        <div>面积:</div>
      ...
      <div class="row renovation-list" >
        <div class="col-md-3 list-con" v-for="item in artData" v-on:click="toDetail(item)">
          ...
          <div class="case-bt">
            ...
            <div class="row">
              <div class="col-md-6">{{item.city}}-{{item.area}}㎡</div>
              <div class="col-md-6">预算-{{item.yusuan}}</div>
            </div>
          </div>
        </div>
...
<style src="../assets/sass/callForeMan.scss" scoped lang="scss" type="text/css"></style>
<style scoped>
  .active {
    background:#57a126;
    color:#fff;
